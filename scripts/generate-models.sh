...
#PREDICTION_METHOD="polynomial" # Method to predict CPU power consumption
#
...
PREDICTION_METHOD="polynomial"

TRAIN_LOAD="all"
TRAIN_LOAD_EXECUTION="dft"
TRAIN_CORES_DISTRIBUTION=("General") #"Group_P" "Group_P_and_L" "Group_1P_2L" "Spread_P" "Spread_P_and_L" "General")
...
for NODE in "${NODES[@]}"; do
  ...
  # Generate one model per specified cores distribution
 	for CORES_DISTRIBUTION in "${TRAIN_CORES_DISTRIBUTION[@]}"; do
      ...
      rm -rf "${OUT_DIR}"
      cpu-power-model -b "${BUCKET}" -n "${CORES_DISTRIBUTION}" -p ${PREDICTION_METHOD} --vars user_load,system_load,freq -t "${TRAIN_FILE}" -a "${TEST_FILES}" -o "${OUT_DIR}"
 	done
done
